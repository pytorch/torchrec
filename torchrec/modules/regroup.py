...
class KTRegroupAsDict(torch.nn.Module):
    ...
    def __init__(self, groups: List[List[str]], keys: List[str]) -> None:
        ...
        self.register_buffer("_permutes", torch.empty(2), persistent=False)
        self.register_buffer("_in_shapes", torch.empty(2), persistent=False)
        self.register_buffer("_out_shapes", torch.empty(2), persistent=False)
