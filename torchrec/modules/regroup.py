...
class PermuteMultiEmbedding(torch.nn.Module):
    ...
    def set_device(self, device: str) -> None:
        self._permute_tensor = none_throws(self._permute_tensor).to(device)
        self._in_shapes = none_throws(self._in_shapes).to(device)
        self._out_shapes = none_throws(self._out_shapes).to(device)
...
class KTRegroupAsDict(torch.nn.Module):
    ...
    def forward(self, keyed_tensors: List[KeyedTensor]) -> Dict[str, torch.Tensor]:
        if not self._is_inited:
            ...
            self.device = keyed_tensors[0].device().type
            assert all(
