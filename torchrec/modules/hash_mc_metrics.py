...
class ScalarLogger(torch.nn.Module):
    ...
    def forward(
        ...
        if self.should_report():
            ...
            Adhoc.writer().add_scalar(
                self.build_metric_name("insert_rate", run_type),
                insert_rate,
                self._scalar_logger_steps,
            )
            Adhoc.writer().add_scalar(
            ...
            Adhoc.writer().add_scalar(
                self.build_metric_name("eviction_rate", run_type),
                eviction_rate,
                self._scalar_logger_steps,
            )
            Adhoc.writer().add_scalar(
                self.build_metric_name("opt_in_rate", run_type),
                opt_in_rate,
                self._scalar_logger_steps,
            )
            Adhoc.writer().add_scalar(
            ...
            Adhoc.writer().add_scalar(
                self.build_metric_name("table_usage_ratio", run_type),
                table_usage_ratio,
                self._scalar_logger_steps,
            )
