...
class CrossNet(torch.nn.Module):
    ...
    (NxN), such that crossing effect can cover all bits on each layer. On each layer l,
    the tensor is transformed into:
...
class LowRankCrossNet(torch.nn.Module):
    r"""
    Low Rank Cross net is a high-efficient cross net. Instead of using full rank cross
    matrix (NxN) at each layer, it will use two kernels :math:`W (N * r)` and
    ...
    NOTE:
        rank `r` should be chosen smartly. Usually, we should expect `r < N/2` to have
        computation saving; we should expect :math:`r ~= N/4` to perserve the accuracy
        of full rank cross net.
...
class VectorCrossNet(torch.nn.Module):
    ...
    (Nx1). So, we will use dot operation to compute the "crossing" effect of features;
    thus, we can save two matrix multiplications to further reduce computational cost
    and cut the number of learnable parameter number.
...
class LowRankMixtureCrossNet(torch.nn.Module):
    r"""
    LowRankMixtureCrossNet is a DCN V2 implementation from the `paper
    <https://arxiv.org/pdf/2008.13535.pdf>`_:

    LowRankMixtureCrossNet defines the learnable crossing parameter per layer as
    low-rank matrix :math:`(N*r)` together with mixture of expert. Compared to
    ...
    a different subspaces, and adaptively combine the learned crosses using a gating
    ...
    where :math:`U_l_i (N, r)`, :math:`C_l_i (r, r)` and :math:`V_l_i (r, N)` are
    low-rank matrix, :math:`*` means element-wise multiplication, :math:`x` means matrix
    multiplication, and :math:`g()` is the non-linear activation function.

    One optimization is when num_expert is 1, the gate evaluation and MOE will be
    skipped for computation saving.
