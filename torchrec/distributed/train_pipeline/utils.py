...
class NodeArgsHelper:
    ...
    def _handle_placeholder(
        ...
        if hasattr(child_node, "ph_key"):
            ...
            for key in ph_keys:
                if "]" in key:
                    arg_info.add_input_attr(key[:-1], True)
                else:
                    arg_info.add_input_attr(key, False)
        else:
            # no-op
            arg_info.add_noop()
        return arg_info
    ...
    def _get_node_args_helper_inner(
        ...
        while True:
            ...
            if child_node.op == "placeholder":
                ...
                fn_module = child_node.target.__module__
                # pyre-fixme[16]
                fn_name = child_node.target.__name__
                match (fn_module, fn_name):
                    ...
                    case ("_operator", "getitem"):
                        # pyre-fixme[6]: For 2nd argument expected `str` but got Unknown
                        arg_info.add_input_attr(child_node.args[1], True)
                        arg = child_node.args[0]
                    case ("torch.utils._pytree", "tree_unflatten"):
                        """
                    ...
                    case ("torchrec.sparse.jagged_tensor", "KeyedJaggedTensor"):
                        call_module_found = False

                        for arg_node in chain(
                            child_node.args, child_node.kwargs.values()
                        ):
                            if isinstance(
