...
class SparseDataDistUtil(Generic[In]):
    ...
    # So we just manage contexts and batches together in lockstep. `forward_hook` is critical
    # for that, check the comment there.
    #
    ...
    #
    # SDD Util (both with and without prefetch) gets away with just two contexts:
    # * context[0] is always the "current batch" context - used for prefetch (here) and model forward (outside this class)
    # * context[1] is always the "next batch" context - used for start/wait_sparse_data_dist
    ...
    def _next_context(self) -> TrainPipelineContext:
        return self._contexts[1]

    def _context_for_model_forward(self) -> TrainPipelineContext:
        context = self._current_context()
        if self._WITH_CONTEXT_ASSERTIONS and self.initialized:
            target_fqns = self._pipelined_modules_fqns()
            if self._with_prefetch:
                assert isinstance(context, PrefetchTrainPipelineContext)
                assert context.module_input_post_prefetch.keys() == target_fqns
                assert context.module_contexts_post_prefetch.keys() == target_fqns
            else:
                assert context.input_dist_tensors_requests.keys() == target_fqns
                assert context.module_contexts.keys() == target_fqns
        return context
    ...
    def _wait_dist_context(self) -> TrainPipelineContext:
        ...
        if self._WITH_CONTEXT_ASSERTIONS and self.initialized:
            if self._have_pipelined_modules:
                assert (
                    len(context.fused_splits_awaitables) > 0
                ), f"fused_splits_awaitables was empty on {context.index=} - was start_sparse_data_dist called?"
        return context
    ...
    def _prefetch_context(self) -> PrefetchTrainPipelineContext:
        ...
        if self._WITH_CONTEXT_ASSERTIONS and self.initialized:
            target_fqns = self._pipelined_modules_fqns()
            assert ctx.input_dist_tensors_requests.keys() == target_fqns
            assert ctx.module_contexts.keys() == target_fqns
        return ctx
    ...
    def _initialize(self, batch: In) -> None:
        # Step 0: initialize two initial contexts
        ...
        def forward_hook(
            ...
            # StagedPipeline and this class (see D59786807 for details)
    ...
    def start_sparse_data_dist(self, batch: In) -> In:
        ...
        next_ctx = self._start_dist_context()
        with record_function(f"## start_sparse_data_dist {next_ctx.index} ##"):
            with use_context_for_postprocs(self._pipelined_postprocs, next_ctx):
                _start_data_dist(self._pipelined_modules, batch, next_ctx)
    ...
    def wait_sparse_data_dist(self) -> None:
        ...
        """
        context = self._wait_dist_context()
    ...
    def prefetch(self, batch: In) -> In:
        ...
        prefetch_context: PrefetchTrainPipelineContext = self._prefetch_context()
        ...
        with self._stream_context(self.prefetch_stream):
            data_per_pipelined_module = _prefetch_embeddings(
                ...
                self._pipelined_modules,
                self._device,
                self._stream_context,  # pyre-ignore[6]: stream_context in _prefetch_embeddings seems to be mis-typed
            ...
            for sharded_module in self._pipelined_modules:
                ...
                prefetch_context.module_input_post_prefetch[forward._name] = data
                prefetch_context.module_contexts_post_prefetch[forward._name] = (
                    prefetch_context.module_contexts.pop(forward._name)
                )
        return batch
