...
class EmbeddingStats(Stats):
    ...
    def log(
        ...
        for rank, device in enumerate(topology.devices):
            ...
            rank_input = f"{stats[rank]['input_sizes']:.2f}"
            rank_output = f"{stats[rank]['output_sizes']:.2f}"
            rank_shards = " ".join(
    ...
    def _get_shard_stats(
        ...
        Returns:
            ranks: list of ranks.
            input_sizes: input sizes in MB across ranks for given shard.
            output_sizes: output sizes in MB across ranks for given shard.
        """

        ranks = list(range(world_size))

        global_batch_size = world_size * sharding_option.batch_size
        ...
        input_sizes = [global_batch_size * pooling_factor * input_data_type_size]

        embedding_dim = sharding_option.tensor.shape[1]
        ...
        if (
            shard.sharding_type == ShardingType.DATA_PARALLEL.value
            or shard.sharding_type == ShardingType.ROW_WISE.value
        ):
            input_sizes = input_sizes * len(ranks)
            output_sizes = output_sizes * len(ranks)
