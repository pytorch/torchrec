...
class TestAutoPlannerWithScaleupProposer(unittest.TestCase):
    ...
    def test_planner_with_virtual_table(self) -> None:
        ...
        # L1 cache size is 64GB per shard and L2 cache size is 128MB per shard
        # Hence for 2 shards, the sizes are doubled but shared by both tables
        self.assertTrue(
            any(
                "dram_virtual_table: HBM: 0.251 GB, DDR: 128.0 GB" in line
                for line in stats
            )
        )
        self.assertTrue(
        ...
        self.assertTrue(
            any("Max HBM: 0.131 GB on ranks [0, 1]" in line for line in stats)
        )
        self.assertTrue(
            any("Min HBM: 0.131 GB on ranks [0, 1]" in line for line in stats)
        )

        # test with multiple ECs
