...
class ReduceScatterV_Req(Function):
    ...
    def forward(
        ...
        with record_function("## _reduce_scatter_v ##"):
            req = dist._reduce_scatter_v(
                output, input, rsi.input_splits, group=pg, async_op=True
            )
...
class ReduceScatterV_Wait(Function):
    ...
    def backward(ctx, grad_output: Tensor) -> Tuple[None, None, Tensor]:
        ...
        with record_function("## _reduce_scatter_v_bw (_all_gather_v) ##"):
            req = dist._all_gather_v(
                grad_input,
                grad_output.contiguous(),
                rsi.input_splits,
                group=ctx.pg,
