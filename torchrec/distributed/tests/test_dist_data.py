...
def _generate_sequence_embedding_batch(
    ...
    for k, v in tensor_by_feature.items():
        in_tensor.append(torch.Tensor(v))

    for k, v in tensor_by_rank.items():
...
class SeqEmbeddingsAllToAllTest(MultiProcessTestBase):
    ...
    def _run_test_dist(
        ...
        qcomms_config: Optional[QCommsConfig] = None,
        variable_batch_size: Optional[bool] = False,
        ...
        res = a2a(
            _input,
            lengths_after_sdd_a2a,
            input_splits,
            output_splits,
            batch_size_per_rank if variable_batch_size else None,
        ).wait()
