...
from torchrec.distributed.sharding.dynamic_sharding_utils import extend_shard_name
...
def create_test_initial_state_dict(
    ...
    initial_state_dict = {}
    for i in range(num_tables):
        extended_name = extend_shard_name(table_name(i))
...
def _test_ebc_resharding(
    ...
    with MultiProcessContext(rank, world_size, backend, local_size) as ctx:
        ...
        new_module_sharding_plan = output_sharding_plan_delta(
        ...
        resharded_m2 = sharder.reshard(
            ...
            changed_shard_to_params=new_module_sharding_plan,
            env=env,
            device=ctx.device,
        )
...
class MultiRankDynamicShardingTest(MultiProcessTestBase):
    def _run_ebc_resharding_test(
        ...
        initial_state_dict = create_test_initial_state_dict(
            num_tables, data_type, embedding_dim, num_embeddings
        )
