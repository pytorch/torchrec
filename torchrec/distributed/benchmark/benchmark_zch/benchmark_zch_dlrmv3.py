...
from generative_recommenders.modules.dlrm_hstu import DlrmHSTU
...
from torchrec.modules.mc_adapter import (
    McEmbeddingBagCollectionAdapter,
    McEmbeddingCollectionAdapter,
)
...
def main(rank: int, args: argparse.Namespace, queue: multiprocessing.Queue) -> None:
    ...
    gin.parse_config_file(
        "/data/users/lizhouyu/fbsource/fbcode/torchrec/distributed/benchmark/movielens_1m.gin"
    )
    ...
    if args.use_zch:
        ec = McEmbeddingCollectionAdapter(  # TODO: add switch for other ZCH or no ZCH
        ...
        ec = EmbeddingCollection(
            tables=list(table_config.values()), device=torch.device("meta")
        )
    ...
    # replace the model._embedding_collection with McEmbeddingCollectionAdapter if using zch
    if args.use_zch:
        model._embedding_collection = ec
    ...
    apply_optimizer_in_backward(
        embedding_optimizer,
        model._embedding_collection.parameters(),
        optimizer_kwargs,
    )
    ...
    train_dataloader, test_dataloader = make_train_test_dataloaders(
        ...
        embedding_table_configs=table_config,
    )

    print("built and sharded model: ", model)
    ...
    if args.use_zch:
        benchmark_probe = BenchmarkMCProbe(
            mcec=model._dmp_wrapped_module.module._embedding_collection.mc_embedding_collection._managed_collision_collection._managed_collision_modules,
            mc_method="mpzch",
            rank=rank,
        )

    interval_num_batches_show_qps = 10
    ...
    for epoch_idx in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            # forward pass
            starter.record()
            _, __, aux_losses, mt_target_preds, mt_target_labels, mt_target_weights = (
                model(batch.uih_features_kjt, batch.candidates_features_kjt)
            )
            ender.record()
            sum(aux_losses.values()).backward()
            # do statistics
            num_queries_per_batch = 0
            for feature_name, feature_jt in batch.uih_features_kjt.to_dict().items():
                num_queries_per_batch += len(feature_jt.values())
            for (
                feature_name,
                feature_jt,
            ) in batch.candidates_features_kjt.to_dict().items():
                num_queries_per_batch += len(feature_jt.values())
        ...
        # TODO: fix the torchmetrics version mismatch between torchrec and generative_recommenders, then uncomment the following code
...
def evaluation(
    model: DistributedModelParallel, data_loader: DataLoader, device: torch.device
) -> Dict[str, float]:
    """
