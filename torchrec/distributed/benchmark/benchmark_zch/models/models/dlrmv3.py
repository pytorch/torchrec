...
class DLRMv3(nn.Module):
    ...
    def forward(
        self,
        batch: Dict[str, KeyedJaggedTensor],
    ) -> Tuple[
        torch.Tensor,
        Tuple[
            torch.Tensor,
...
def make_model_dlrmv3(
    ...
    hstu_config.contextual_feature_to_min_uih_length = configs[
    ...
    for i in range(len(configs["multitask_configs"])):
        if configs["multitask_configs"][i]["task_type"] == "regression":
            ...
            task_type_list.append(MultitaskTaskType.CLASSIFICATION)
        else:
            raise ValueError(
