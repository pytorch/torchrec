...
def benchmark(
    ...
    total_duration_sec = elapsed_time.mean().item() * 1e-3  # time in seconds
    std_sec = elapsed_time.std().item() * 1e-3  # time in seconds
    gc.enable()
...
def write_report(
    ...
    for benchmark_res in benchmark_results:
        avg_dur_s, std_dur_s = benchmark_res.total_duration_sec, benchmark_res.std_sec
...
def main() -> None:
    ...
    num_requests = len(bench_inputs) * batch_size * num_benchmarks
