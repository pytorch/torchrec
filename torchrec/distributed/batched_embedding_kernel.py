...
def _get_sharded_local_buckets_for_zero_collision(
    ...
    for table in embedding_tables:
        ...
        extra_buckets = int(local_rank < (total_num_buckets % world_size))
        bucket_offset_start = (
            total_num_buckets // world_size + extra_buckets
        ) * local_rank
        bucket_offset_end = min(
            total_num_buckets,
            (total_num_buckets // world_size + extra_buckets) * (local_rank + 1),
        )
        bucket_size = (
