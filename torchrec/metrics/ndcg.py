...
class NDCGComputation(RecMetricComputation):
    ...
    def __init__(
        ...
        *args: Any,
        ...
        apply_negative_task_mask: bool = False,
        ...
        **kwargs: Any,
        ...
        self._apply_negative_task_mask: bool = apply_negative_task_mask
    ...
    def _get_adjusted_ndcg_inputs(
        ...
        *,
        ...
        example_corresponds_to_session_with_length_greater_zero = example_to_length > 1

        # Remove all single-length sessions.
        return (
            predictions[:, example_corresponds_to_session_with_length_greater_zero],
            labels[:, example_corresponds_to_session_with_length_greater_zero],
            weights[:, example_corresponds_to_session_with_length_greater_zero],
            converted_session_ids[
                example_corresponds_to_session_with_length_greater_zero
            ],
        )
    ...
    def _get_ndcg_states(
        ...
        *,
        ...
        """
        Venkat's "Inefficient" Normalized Discounted Cumulative Gain (NDCG) @ k.

        TODO: Refactor into smaller helper functions :)
        """
        # Remove all single-length sessions from all variables.
        if self._remove_single_length_sessions:
            (
                adjusted_predictions,
                adjusted_labels,
                adjusted_weights,
                adjusted_session_ids,
            ) = self._get_adjusted_ndcg_inputs(
        ...
        # Convert session IDs to [num_tasks, num_sessions].
        ...
        _, sorted_predictions_indices = adjusted_predictions.sort(
            descending=True, dim=-1
        )
        ...
        label_by_prediction_ranks = (sorted_session_ids_by_predictions_mask).cumsum(
            dim=-1
        )
        ...
        ideal_gains = (
            expanded_sorted_labels_by_labels * sorted_session_ids_by_labels_mask
        )
        observed_gains = (
            expanded_sorted_labels_by_predictions
            * sorted_session_ids_by_predictions_mask
        )
        ...
        ndcg_report = (1 - ndcg).to(device=labels.device)
    ...
    def _compute_ndcg(
        self, *, sum_ndcg: torch.Tensor, num_sessions: torch.Tensor
    ) -> torch.Tensor:
    ...
    def update(
        ...
        **kwargs: Dict[str, Any],
        ...
        if self._apply_negative_task_mask and self._is_negative_task_mask is not None:
            print(f"{self._is_negative_task_mask=}")
...
class NDCGMetric(RecMetric):
    ...
    def _get_task_kwargs(
        ...
        for task_info in all_task_info:
            is_negative_task_mask.append(task_info.is_negative_task)
        return {
            "is_negative_task_mask": is_negative_task_mask,
        }
